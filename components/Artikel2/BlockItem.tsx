...
interface Props {
  block: EditorBlock;
  updateBlock: (id: string, updates: Partial<EditorBlock>) => void; // <-- Partial<EditorBlock>
...
export default function BlockItem({
  ...
  const commit = () => {
    if (contentRef.current !== block.content) updateBlock(block.id, { content: contentRef.current });
  };
  ...
  const RenderView = () => {
    switch (block.type) {
      ...
      case "code":
        return (
          <pre className="rounded bg-gray-900 text-gray-100 p-3 text-sm overflow-auto">
            {block.language && (
              <div className="text-xs text-gray-400 mb-1">// Sprache: {block.language}</div>
            )}
            {block.content}
          </pre>
        );
  ...
  const languages = [
    "javascript","typescript","python","java","csharp","cpp","ps1",
    "php","go","rust","sql","html","css","json","bash","yaml","markdown"
  ];
    ...
    >
      <div className={`absolute top-2 right-2 z-10 flex items-center gap-1 bg-white/90 backdrop-blur rounded-lg border shadow-sm px-1.5 py-1 ${active ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition`}>
        ...
        <select
          data-ctl
          value={block.type}
          onChange={(e) => convertType(block.id, e.target.value as EditorBlockType)}
          title="Convert type"
        ...
        {block.type === "code" && (
          <select
            ...
            onChange={(e) => updateBlock(block.id, { language: e.target.value })} // <-- jetzt korrekt
